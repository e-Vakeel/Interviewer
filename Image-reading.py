...
class Phi:
    def __init__(self, device):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(model_id, device_map=device, trust_remote_code=True, torch_dtype=torch.bfloat16, attn_implementation="flash_attention_2")
        self.model = torch.compile(self.model, mode='max-autotune')
        self.model = torchao.autoquant(self.model)

        self.processor = AutoProcessor.from_pretrained(model_id, trust_remote_code=True, num_crops=16)  # for best performance, use num_crops=4 for multi-frame, num_crops=16 for single-frame.
        self.device = device
